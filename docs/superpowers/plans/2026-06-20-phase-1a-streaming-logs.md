...
**Scope note (intentional simplification):** the spec's "search-match highlight" and "jump-to-latest FAB" are implemented in this slice as **search filtering** (matching lines only) + **stdout/stderr coloring** + **autoscroll while following**. In-line substring highlight and a dedicated jump-to-latest FAB are deferred as polish — they don't change the architecture and can be added without touching the streaming layer. Flag at review if you want them pulled into this slice.
